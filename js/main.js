...
var dailySongs = []
var dailyPlayList = []
fetch(cloud + 'login/status?timetamp=' + (Number(new Date())))
  .then(r => r.json())
  .then(r => {
...
function mySelfGui() {
  document.getElementById('playerBoxIndex').innerHTML = '<div class="prf-User-Top-Title"><img id="largerUserHeadImg" src="" alt="" srcset=""><h1 id="page-userNickName"></h1></div><div class="row-index"><h1>我的音乐 </h1><div class="prf-mainBox"><div><div class="prf-row-playList row-Mylove"><div id="row-Mylove"></div>你欢<br>喜的 <div class="abPlayButton" id="MylovePlayButton"></div></div></div><div style="display: grid;grid-template-columns: repeat(1, 1fr);gap: 20px;"><div id="row-myMusicEverDay" class=" prf-row-playList-long prf-row-playList "><div onclick="funDailySongs(true)" id="row-EverDay"></div>每日推荐 <div class="abPlayButton" onclick="funDailySongs(false)"></div></div><div style="display: flex;gap: 20px;"><div id="row-myMusicYoursRadio" class=" prf-row-playList-other prf-row-playList">私人<br>FM <div class="abPlayButton" id="myFMPlayButton"></div></div><div id="prf-row-myRader" class=" prf-row-playList-other prf-row-playList "><div id="row-myRader"></div>私人<br>雷达 <div class="abPlayButton" id="myRaderPlayButton"></div></div></div> </div></div></div></div>'
  ...
  fetch(cloud + 'user/subcount')
    .then(r => r.json())
    .then(subcount => {
      fetch(cloud + 'user/playlist?uid=' + uuid)
        .then(r => r.json())
        .then(playlist => {
          ...
          fetch(cloud + 'recommend/resource')
            .then(r => r.json())
            .then(dailyPlayListPost => {
              dailyPlayList = dailyPlayListPost.recommend
              fetch(cloud + 'playlist/detail?id=' + dailyPlayList[0].id)
                .then(r => r.json())
                .then(res => {
...
function loginGui(title) {
  ...
  document.getElementById('playerBoxIndex').innerHTML = '<h1>' + title + '</h1><input id="phone" type="text" placeholder="请输入手机号"><br><input id="password" type="password" placeholder="请输入密码"><br><button id="loginButtom" class="whitebutton" onclick="loginButton()" style="height: 48px;">开始认证</button>'
}
...
function SearchChuLi(songslab, row) {
  ...
  if (songslab != undefined) {
    ...
    fetch(cloud + 'song/detail?ids=' + musicIds)
      ...
      .then(data => {
        ...
        fetch(cloud + 'song/url?id=' + musicIds)
          .then(response => response.json())
          .then(data => {
    ...
    .then(res => {
      ...
      fetch(cloud + 'playlist/detail?id=' + res.playlist.id)
        ...
        .then(res => {
          ...
          fetch(cloud + 'song/detail?ids=' + muListSelecttrackIds)
            .then(response => response.json())
            .then(data => {
              if (chageUI) {
              ...
              fetch(cloud + 'song/url?id=' + muListSelecttrackIds)
                .then(response => response.json())
                .then(data2 => {
